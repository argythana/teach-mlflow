...
- [x] **Traces orientation note** — **done.** Shipped as a markdown cell in `k_`'s wrap-up ("The Traces tab — orientation"): what the (always-empty-for-traditional-ML) Traces tab is, why it's empty here, and that it lights up only for the GenAI track. In-scope UI literacy, not a tracing tutorial.
